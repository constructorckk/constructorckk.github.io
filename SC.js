...
function Spisenie() {
	...
	for (var i = 0; i < colorants.length-1; i++) {
		...
		tab2_row_2_dat_2_int.value = (String((colorants[i+1].getElementsByClassName('Chem_M')[0].value * 1.00 / 1000).toFixed(2)) + ' кг');
	...
	for (var i = 0; i < cmemyy.length-1; i++) {
		...
		for (var r = 2; r < tab_trs.length; r++)	{
			...
			if (add_ind == true) {
				...
				tab2_row_2_dat_2_int.value = (String((cmemyy[i+1].getElementsByClassName('Chem_M')[0].value * 1.00 * for_30acid / 1000).toFixed(2)) + ' кг');
